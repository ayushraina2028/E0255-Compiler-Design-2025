...
// Implementation of your methods - copy your existing code here but adapt it to
// fit the class definition
void HoistAnticipatedExpressionsPass::printSets(
    Function &F, DenseMap<BasicBlock *, BitVector> &Gen,
    DenseMap<BasicBlock *, BitVector> &Kill,
    DenseMap<BasicBlock *, BitVector> &IN,
    DenseMap<BasicBlock *, BitVector> &OUT, unsigned int nextBitIndex) {
  // Your implementation copied from original code
  for (auto &BB : F) {
...
int HoistAnticipatedExpressionsPass::hoistAnticipatedExpressions(
    Function &F, DenseMap<BasicBlock *, BitVector> &OUT,
    DenseMap<Expression, unsigned int> &ExpressionToBitMap, DominatorTree &DT) {
...
//      outs() << "Hoisting to block: " << BB.getName() << "\n";
      ...
      if (opcode == Instruction::SRem) {
        ...
        if (opcode == Instruction::SIToFP) {
          newInst = Builder.CreateSIToFP(op1, Type::getDoubleTy(F.getContext()),
                                         hoistedName);
        } else if (opcode == Instruction::FPToSI) {
          newInst = Builder.CreateFPToSI(op1, Type::getInt32Ty(F.getContext()),
                                         hoistedName);
        } else {
          newInst = Builder.CreateUIToFP(op1, Type::getDoubleTy(F.getContext()),
                                         hoistedName);
        }
        ...
        newInst = Builder.CreateBinOp(
            static_cast<Instruction::BinaryOps>(opcode), op1, op2, hoistedName);
      }
          ...
          if (auto *binOp = dyn_cast<BinaryOperator>(&I)) {
            ...
            if (binOp->getOpcode() == opcode &&
                ((curOp1 == op1 && curOp2 == op2) ||
                 (binOp->isCommutative() && curOp1 == op2 && curOp2 == op1))) {
   ...
   //       outs() << "  op2: " << *op2 << "\n";
     ...
     //       outs() << "  Adding new expression at index " << nextBitIndex
       //            << "\n";
          ...
          Expression exprKey =
              make_tuple(Instruction::CastOps(Cast->getOpcode()), op, nullptr);
